...
def mock_resolve_project(mocker):
    return mocker.patch("faculty_cli.cli._resolve_project", return_value=PROJECT.id)
...
def test_dataset_get(mocker, mock_resolve_project):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["dataset", "get", "test-project", "source", "dest"])
...
def test_dataset_get_source_not_found(mocker, mock_resolve_project):
    ...
    assert result.output == "No such object missing-source in project test-project\n"


@pytest.mark.parametrize("exception", [FileNotFoundError, NotADirectoryError, OSError])
...
def test_dataset_put(mocker, mock_resolve_project):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["dataset", "put", "test-project", "source", "dest"])
    assert result.exit_code == 0
...
def test_dataset_mv(mocker, mock_resolve_project):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["dataset", "mv", "test-project", "source", "dest"])
    assert result.exit_code == 0
...
def test_dataset_cp(mocker, mock_resolve_project):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["dataset", "cp", "test-project", "source", "dest"])
    assert result.exit_code == 0
...
def test_dataset_rm_object_not_found(mocker, mock_resolve_project):
    ...
    runner = CliRunner()
    result = runner.invoke(
        cli, ["dataset", "rm", "test-project", "object"]
    )
...
def test_dataset_ls(mocker, mock_resolve_project):
    ...
    mock_ls = mocker.patch("faculty.datasets.ls", return_value=["/", "/first-object", "/second-object"])

    runner = CliRunner()
    result = runner.invoke(cli, ["dataset", "ls", "test-project", "--prefix", "/", "--show-hidden"])
